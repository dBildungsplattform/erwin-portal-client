...
describe('PersonDetailsView', () => {
  ...
  test('it does not show password reset for non-lehr rollenart', async () => {
    personStore.personenuebersicht = mockPersonenuebersicht;
    setCurrentPerson(EmailAddressStatus.Enabled);
    await nextTick();

    expect(wrapper?.find('[data-testid="device-password-info"]').exists()).toBe(false);
  });
  test('it disables klasse change button if no checkbox is selected', async () => {
    await wrapper?.find('[data-testid="zuordnung-edit-button"]').trigger('click');
    await nextTick();

    expect(wrapper?.find('[data-testid="klasse-change-button"]').attributes('disabled')).toBeDefined();
  });

  test('it disables delete button if no checkbox is selected', async () => {
    await wrapper?.find('[data-testid="zuordnung-edit-button"]').trigger('click');
    await nextTick();

    expect(wrapper?.find('[data-testid="open-zuordnung-delete-dialog-button"]').attributes('disabled')).toBeDefined();
  });

  test('it does not show loading spinner if not loading', async () => {
  ...
  test('it disables save button if no changes were made', async () => {
    await wrapper?.find('[data-testid="zuordnung-edit-button"]').trigger('click');
    await nextTick();
  ...
  test('it disables submit button in add Zuordnung form if required fields are missing', async () => {
    ...
    const submitButton: Element | null = document.body.querySelector('[data-testid="zuordnung-creation-submit-button"]');
    if (submitButton) {
  ...
  test('it disables submit button in change befristung form if no date is selected', async () => {
    await wrapper?.find('[data-testid="zuordnung-edit-button"]').trigger('click');
    await nextTick();
    ...
    const submitButton: Element | null = document.body.querySelector('[data-testid="change-befristung-submit-button"]');
    if (submitButton) {
      expect(submitButton.hasAttribute('disabled')).toBe(true);
    }
  });
